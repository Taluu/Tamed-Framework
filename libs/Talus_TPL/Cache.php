...
class Cache implements Interfaces\Cache {
  ...
  public function dir($dir = null) {
    if ($dir !== null) {
      ...
      if (!is_dir($dir)){
        throw new Exceptions\Dir(array('Dossier "%s" non existant.', $dir));
        return false;
      }
  ...
  public function put($data) {
    ...
    if (!$lock){
      throw new Exceptions\Write('Ecriture en cache impossible');
      return false;
    }
  ...
  public function exec(Talus_TPL $tpl) {
    ...
    if ($file === array()) {
      throw new Exceptions\Exec('Vous venez d\'essayer d\'executer aucun fichier...');
      return false;
    }
   ...
   *
   * @param Main $tpl Objet TPL à utiliser lors de la lecture du cache
   * @return bool
   *
   * @see self::exec()
   */
